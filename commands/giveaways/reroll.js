...
const notend = client.modules.embed(client, client.colors.red, `${client.emotes.cross} | **Giveaway is still running.**`)
const nofound = client.modules.embed(client, client.colors.red, `${client.emotes.cross} | **No Giveaway Found.**`)
const success = client.modules.embed(client, client.colors.green, `${client.emotes.check} | **Giveaway rerolled.**`)
...
client.giveawaysManager.reroll(giveaway.messageId).then(() => { interaction.reply({ embeds: [success] }) }).catch((e) => { interaction.reply({ embeds: [error] }) })
}}