module.exports = { 
    ...
    switch(option) {
        case "lock": 
        if(!channel.permissionsFor(message.guild.id).has("CONNECT")) { return interaction.reply({ embeds: [alreadyl] }) }
        ...
        break;
        case "unlock":
        if(channel.permissionsFor(message.guild.id).has("CONNECT")) { return interaction.reply({ embeds: [alreadyu] }) }    
        channel.permissionOverwrites.edit(interaction.guild.id, {
            CONNECT: true
        })            
